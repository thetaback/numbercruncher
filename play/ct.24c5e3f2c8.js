...
(() => {
  ...
  var resLib = {
    ...
    async loadGame() {
      ...
      const atlases = [
        ["./img/a0.{webp,png}.df3de3a75f.json","./img/a1.{webp,png}.eb83bd3e2f.json"]
      ][0];
      const tiledImages = [
...
// Is the operation division?
    ...
    if (Math.abs(this.currentvalue%other.amount) == 0) {
        ...
        other.cgroup = "Solid" // turns operation into a wall
        killtrue = false
    }
        ...
        /* template Exit — place_collisionTemplate (Collision with a template event) */
{
    ...
    // if (rooms.current.nextRoom) {
        ...
        tween.add({
            ...
            if (isMultiPlayer && rooms.current.nextRoom==undefined) {   
                let serverendtime=new Date().toISOString().replace("T", " ");
                updateEndTime(sessionStorage.game_user_id, serverendtime).then(()=> {
                    window.location.href = "../scoreboardview.html"
                });
            } else {
...
if (isMultiPlayer) {
    ...
    updateStartTime(sessionStorage.game_user_id, serverstarttime, levelseq[0]);
}
}
...
function roomStart(room) {
    ...
    if (isMultiPlayer && nextLevelIndex != 1) {
        updateCurrentLevel(sessionStorage.game_user_id, room.name);
    }
}
